# serial-debugger — 跨平台串口调试工具 v0.8.0
...
**稳定基线版本：把 0.7.5.1 ～ 0.7.5.10 的全部修复整体转入 `main`。本版不含功能改动** —— 源码与 0.7.5.10 的区别只有版本号，四个 exe 的行为与 0.7.5.10 完全一致。

- **外部测试结论**（0.7.5.10 轮报告，机器可复核）：`sendone` 逐字节验收通过（默认剥头 **4 B**、`--raw` 对照 **9 B** 含 5 字节头，正面展示了修复前的错误形态）；标签页修复原场景未复现，且 **5 轮**启停 `create idle` 总增量 = **5**（等于自动建会话次数；若回归会是 10）；全量回归全绿；`delay` 语义未回退（`0` 256 ms vs `1000` 9 244 ms）。
- 测试方建议「以 0.7.5.10 为稳定基线」，本版即按此发布，版本号进位到 0.8.0 表示该系列收口。
- **已知未修（留给 0.8.1）**：TODO #77 —— `autosend status` 的 `queueCount` 实为**字节数**（每条 = 内容长度 + 7），而 `entriesCount` 在 CLI 路径恒为 0；属命名歧义陷阱，非功能缺陷，也非本系列引入。
- 测试方同意把「设置页守卫」**移出必验清单**（无可点击入口，属代码级保障）。
