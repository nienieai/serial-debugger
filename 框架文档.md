...
- **CLI**: 交互模式用 3 管道，命令行模式用 CallOnce 临时管道，27 条命令（含 declare）
- **MCP**: Model Context Protocol over stdio，25 个工具覆盖全部功能（含 serial_declare）
...
graph TB
    subgraph "GUI (serial-gui.exe)"
        subgraph "JS 前端"
            ...
            EVENTS[EventsOn: rx, tx, ports-changed, daemon-offline, process-changed]
    ...
    subgraph "MCP (serial-mcp.exe)"
        MCP_TOOLS[24 工具: JSON-RPC 2.0 over stdio]
    end
...
菜单栏「设置 → 软件设置」打开居中模态窗，集中管理全局设置：
- **编码与显示**：编码格式、Hex 前缀/大小写/分隔符、转义字节模式+格式、可见控制字符、允许复制转义字节、默认锁定滚动
- **主题与语言**：颜色主题卡片、图标主题卡片、语言选择
- **存储与行为**：自动保存历史、启动时自动创建会话

`frontend/settingspage.js` 构建 DOM，`renderSettingsThemeGrid()` / `renderSettingsIconGrid()` 渲染主题卡片。
...
| `sendqueue` | `<file> [pid]` | 从文件写多行到 sendq 共享内存 |
| `autosend start` | `<ms> <mode> [pid]` | `autosend.start` |
| `autosend stop` | `[pid]` | `autosend.stop` |
| `autosend status` | `[pid]` | `autosend.status` |
| `autosend interval` | `<ms> [pid]` | `autosend.interval` |
...
内置图标位于 `config/icons/system/`（17 个 24×24 SVG + `icons.json` 元数据），编译时 `//go:embed` 嵌入。外部图标主题放在 `themes/icons/<主题名>/`（文件夹 = 主题，`icons.json` + 平铺 `.svg` 文件），文件名 = 内置图标键名，覆盖对应图标。
...
版本号统一在 `version/version.go`（`const Version = "0.6.3"`），改一处全部同步。
