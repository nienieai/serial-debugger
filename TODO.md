...
| 36 | 注册握手在**并发**建立会话时约 8% 失败 | 已缓解（根因未定位） | v0.7.4.2 处置。真实存在且只在并发下出现：串行 300 次仅 1 次（0.3%），**8 路并行 × 60 轮 = 42/480（8.8%）**，与外部报告测到的 9.3% 吻合。报告推断的根因（客户端监听器未就绪）**不成立**，实测反驳 5 条：① 守护进程日志显示该会话已完成注册，而「已注册」那行位于 `addSession` 内、`addSession` 又在两条回连管道都成功之后；② 没有一条「连接客户端 resp/sub 管道失败」；③ 失败会话未被驱逐（插桩实测 `已驱逐=false`）；④ 守护进程没有写确认失败（新增该日志后实测 0 次）；⑤ 报告称「失败指纹失效（三者计数相等）」实为统计方法问题——失败会话同样会打「已注册」，真正的指纹是「已注册但从未开始通信」，现已成为常驻错误日志。现场矛盾：守护进程侧写成功、客户端侧读 `daemonConn` 失败，指向 `pipe` 层并发下的句柄/缓冲竞态。**处置**：客户端握手失败自动重试 2 次（每次换全新 clientId → 全新管道名，不踩残留），全部失败时错误带尝试次数与原因，失败尝试仍在守护进程侧留痕。实测 **8.8% → 0.21%**（1/480），v0.7.4.2 复测 0/480。**这是缓解不是修复**，根因仍待定位 |
...
| 需求 | 说明 |
|------|------|
| 注册握手并发失败（约 8%） | 复现到可处置程度（8 路并行 8.8%，串行 0.3%），推翻外部报告的根因推断，改为握手失败自动重试（全新 clientId）。实测 8.8% → 0.21%，v0.7.4.2 复测 0/480。根因（`pipe` 层并发竞态）未定位，见「已知问题」#36 |
