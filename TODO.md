...
| P2 | Web 客户端 | HTTP + WebSocket in daemon | 浏览器操作串口，局域网可用，`--web` 开关控制 |
...
| P2 | 文本/Hex 对照显示 | 分段解码已有 | 历史框分栏：Hex+偏移 | 文本对照，选中联动 |
...
| P2 | 上滚召回完善：滚动事件在内容不足时不可触达 | onscroll 替代方案 | display 内容不足以产生滚动条时需额外入口 |
...
| # | 问题 | 状态 | 说明 |
|---|------|------|------|
| 1 | 清空后内容太少时上滚不触发 expandHistory | ⚠️ 部分修复 | display 内容不足以产生滚动条时 onscroll 事件无法触发；已保留可点击的 "↑ 向上滚动加载更多" 入口 |
| 2 | 重复快速点击"加载磁盘历史"可能触发多次请求 | ⚠️ 已加固 | 已加入 `_diskLoading` 互斥和 pointerEvents 禁用，但极快连击下仍有极小概率穿透 |
| 3 | loadTabHistory 重新从 daemon 取数据后 `_clearedAt` 标记丢失 | ⚠️ 已知限制 | 切换标签导致缓存被重新覆盖时清除标记线消失，所有历史均可见；此为预期行为 |

## v0.6.4 已完成

| 需求 | 说明 |
|------|------|
| DecodeEntryContentOnly 误判修复 | 新增 flags（≤0x03）和 delay（5–60000ms）校验，防止 Modbus 首字节 0x01 被误判为 multistr 版本头 |
| send.trigger raw 参数 | 新增 `raw` 参数区分原始发送（raw:true 跳过解码）和 multistr 快捷面板发送（raw:false），修复 Hex 数据首字节 0x01 时字节丢失 |
| 自动发送逐条目延迟 | `sendOneRound` 默认延迟从硬编码 1000ms 改为跟随 `autoSendIntervalMs`，兜底 100ms |
