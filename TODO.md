...
| 50 | **`probe` 的「空结果」有两条静默路径，与「没有设备」不可区分** —— 目标端口无应答时必然撞上 10s IPC 超时，且一次未结束的探测会让后续探测在几十毫秒内报「未检测到」 | 待处理 | v0.7.5 起存在，0.7.5.1 轮由外部测试报告与本地复现共同确认。**（a）耗时与超时**：`probeRead` 的总预算是 `max(3×timeout, 1s)`，默认 `probe.toml` 的 `timeout_ms=200` → 取 1s 下限；端口静默时 `Read` 返回 `(0,nil)` 不满足任何提前退出条件，**每次尝试都跑满这 1s**。实测同一静默端口：1 次尝试 **1.11s**、7 次 **7.48s**、默认 3 规则 × 7 波特率 = 21 次 **10.18s**。而 `client.Call` 的 IPC 请求超时是**硬编码 10s**（`client/client.go:525`）→ CLI 必然报 `request timeout: ports.probe`；守护进程并不停，继续把 21 次扫完（实测后台探测总耗时 **33.2s**）。**（b）假阴性**：这段窗口内再做任何探测都会在**几十毫秒**内返回空结果。报告实测「正确配置 + 编码器在线」也会得到 `未检测到已知设备`（报告 §3.3：58ms，且已用 MCP 证实与客户端无关）；本地复现：被会话占用 46ms、客户端超时后紧接着 61~65ms、而在同一探测**结束后**恢复为 1093~1245ms。**（c）根因有两条，报告只点到了前一条**：① `probe.go:207` 的 `if occupiedPorts[portName] { continue }`（端口被**会话**占用，确定性假阴性）；② `probe.go:239-240` 的 `if err != nil { continue }`——`ProbePorts` 内部对串口的 **open/read 失败被裸 `continue`/`break` 吞掉，连一行日志都没有**。本地因果实验证明②才是超时那条路径的真正机制：`sessions` 显示**没有任何进程**持有 COM3，跟随探测仍在 65ms 内返回空（若走①必须 portMap 里有进程）；且同一探测结束后立刻恢复正常。危害是**静默假阴性**：设备在线并被正确应答，工具却报「未检测到已知设备」，排查者会去怀疑接线/波特率/地址/CRC。**修法（建议一起做）**：① 把两条吞掉的分支都变成显式结果（`skipped: [{port, reason}]`，或请求端口全被跳过时直接报错），并给②补日志；② 给 `ProbePorts` 加**总预算**（如 8s）到点即返回已有结果 + 让 `ports.probe` 的 IPC 超时大于最坏耗时；③ 客户端超时后显式取消、会话断开时中止探测，缩短占用窗口；④ 精简默认波特率矩阵（3×7=21 次是 20s+ 的直接原因）。`probe` 只在 CLI/MCP 暴露（GUI 无此功能），而外部报告正是用 `serial-cli probe` 发现问题的，下一条报告大概率仍会写「probe 不行」 |
